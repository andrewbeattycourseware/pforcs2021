...
def positive_difference(argument1, argument2, data_type):
    if not try_parse(argument1, data_type):
        print(error_msg(
            f'Argument 1 of "positive_difference()" cannot be parsed to {data_type}'))
        return False
    if not try_parse(argument2, data_type):
        print(error_msg(
            f'Argument 2 of "positive_difference()" cannot be parsed to {data_type}'))
        return False
...
def is_valid_file(file_name, valid_file_extensions):
    if len(valid_file_extensions) >= 1:
        if re.match(f'^.*(\\.{"|".join(valid_file_extensions)})$', file_name):
            return True
        print(error_msg(
            f'File extension must be one of the following:\n({", ".join(valid_file_extensions)})'))
        return False
    else:
        return False
...
def build_log_entry_section_dict(log_entry, log_entry_section):
    ...
    log_entry_dict['resource'] = (
        log_entry[0], log_entry[0][:-1])[log_entry[0][-1] == '/']
    if len(log_entry) > 1:
        param_list = list(
            map(lambda x: parse_to_dict_item(x, '='), log_entry[1].split('&')))
        if len(param_list) >= 1:
...
def get_log_entry_section(log_entry, log_entry_section):
    if try_parse(log_entry, str):
        ...
        valid_log_entry_sections = (
            LOG_ENTRY_URL, LOG_ENTRY_HTTP_REQUEST, LOG_ENTRY_DATETIME, LOG_ENTRY_NO_OF_BYTES_DOWNLOADED)
        if log_entry_section in valid_log_entry_sections:
            if log_entry_section is LOG_ENTRY_URL:
                log_entry_list = list(filter(lambda x: re.search(
                    f'{LOG_ENTRY_URL_REGEX}', x), log_entry_list))
                ...
                log_entry_list = list(filter(lambda x: re.search(
                    f'{LOG_ENTRY_HTTP_REQUEST_REGEX}', x), log_entry_list))
                if len(log_entry_list) > 0:
                    log_entry_list = log_entry_list[0]
                    log_entry_list = re.split(
                        f'{LOG_ENTRY_HTTP_REQUEST_REGEX}', log_entry_list)
                    return log_entry_list[1]
                else:
                    return None
            elif log_entry_section is LOG_ENTRY_DATETIME:
                log_entry_list = list(filter(lambda x: re.search(
                    f'{LOG_ENTRY_DATETIME_REGEX}', x), log_entry_list))
                if len(log_entry_list) > 0:
                    log_entry_list = log_entry_list[0]
                    log_entry_list = re.split(
                        f'{LOG_ENTRY_DATETIME_REGEX}', log_entry_list)
                    return log_entry_list[1]
                else:
                    return None
...
def build_dataframe_row(log_entry):
    if log_entry and try_parse(log_entry, str):
        ...
        processed_log_entry = {}
        log_entry_section = get_log_entry_section(
            log_entry, LOG_ENTRY_DATETIME)
        if log_entry_section is not None:
            if try_parse(log_entry_section, datetime, '%d/%b/%Y:%H:%M:%S'):
                processed_log_entry['timeOccurred'] = datetime.strptime(
                    log_entry_section, '%d/%b/%Y:%H:%M:%S')
        log_entry_section = get_log_entry_section(
            log_entry, LOG_ENTRY_HTTP_REQUEST)
        if log_entry_section is not None:
            http_request_param_dict = build_log_entry_section_dict(
                log_entry, LOG_ENTRY_HTTP_REQUEST)['parameters']
            if 'JSESSIONID' in http_request_param_dict:
                processed_log_entry['sessionId'] = http_request_param_dict['JSESSIONID']
        log_entry_section = get_log_entry_section(
            log_entry, LOG_ENTRY_NO_OF_BYTES_DOWNLOADED)
        if log_entry_section is not None:
            if try_parse(log_entry_section, float):
                processed_log_entry['noOfBytesDownloaded'] = float(
                    log_entry_section)
